...
void Vmac_array_tb::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vmac_array_tb::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vmac_array_tb::traceConfig() const {
...
void Vmac_array_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void Vmac_array_tb___024root__trace_init_top(Vmac_array_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vmac_array_tb___024root__trace_decl_types(tracep);
    Vmac_array_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vmac_array_tb___024root__trace_register(Vmac_array_tb___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vmac_array_tb::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vmac_array_tb::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    (void)levels; (void)options; // Prevent unused variable warning
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vmac_array_tb___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
