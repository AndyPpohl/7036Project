    ...
    always_comb begin
        ...
        begin
            ...
            begin
                int idx = (i * DIM) + j;
                if(j == 0) begin
                    assign a_conn[idx] = ain[i];
                end
                else begin
                    assign a_conn[idx] = a_conn[(i * DIM) + (j - 1)];
                end
                
                assign b_conn[idx] = win[i][j];

                if(i == 0) begin
                    assign cin_conn[idx] = cin[j];
                end
                else begin
                    assign cin_conn[idx] = cout_conn[((i-1) * DIM) + j];
                end
